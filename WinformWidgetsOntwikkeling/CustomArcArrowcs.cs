...
namespace WindowsFormsApp1
{
    public partial class CustomArcArrowcs : System.Windows.Forms.Button
    {
        ...
        protected override void OnPaint(PaintEventArgs pe)
        {
            ...
            if (this.swapVal == 0)
            {
                pe.Graphics.DrawLine(pen, rect.X, rect.Y + Padding.Top + 1,
                                    rect.Width / 2 - (Padding.Right + Padding.Left), rect.Y + Padding.Top + 1);
                pe.Graphics.DrawArc(pen, rect.X, rect.Y + Padding.Top, rect.Width - radius / 2,
                                    rect.Height - (Padding.Top + Padding.Bottom),
                    startAngle - 90, endAngle);
                pe.Graphics.DrawLine(pen, rect.X, rect.Y + rect.Height - this.Padding.Bottom,
                                    rect.Width / 2 - (Padding.Right + Padding.Left),
                                    rect.Y + rect.Height - this.Padding.Bottom);
            }
            else {
                pe.Graphics.DrawLine(pen, rect.X+rect.Width / 2 - (Padding.Right + Padding.Left), 
                                     rect.Y + Padding.Top + 1,
                                        rect.X + rect.Width - (Padding.Right + Padding.Left), 
                                        rect.Y + Padding.Top + 1);
                pe.Graphics.DrawArc(pen, rect.X+Padding.Left, rect.Y + Padding.Top, rect.Width - radius / 2,
                                    rect.Height - (Padding.Top + Padding.Bottom)
                     , startAngle + 90, endAngle);
                pe.Graphics.DrawLine(pen, rect.X + rect.Width / 2 - (Padding.Right + Padding.Left),
                                    rect.Y + rect.Height - this.Padding.Bottom,
