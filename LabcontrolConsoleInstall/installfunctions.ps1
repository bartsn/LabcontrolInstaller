...
Function runAllInstallers {
	Param( [Int16]$runGitInstall, [Int16]$run7Zipinstall)
	if ($runGitInstall) {
		installGitScm
	}
	if ($run7Zipinstall) {
...
Function cloneRepo {
	...
	#$process = Start-Process PowerShell { Invoke-Command $cloneCommand }
	$process = Start-Process -FilePath "C:\Program Files\Git\cmd\git.exe" -ArgumentList "clone https://github.com/Lectoraat-DB-S/labcontrol.git" -PassThru
...
Function installRSVISA {
	...
	$process = Start-Process -FilePath "C:\PROGRA~1\7-Zip\7z" -ArgumentList "e c:\temp\RS_VISA_Setup_Win_7_2_5.7z -oc:\temp" -PassThru
	dump2ScrnAndFile "Uitpakproces RS VISA installer met id: $($process.Id) loopt."

	# Wait untill process is finished
	Wait-Process -Id $process.Id
	dump2ScrnAndFile "installer VISA uitgepakt."
	
	dump2ScrnAndFile 'Starten van R&S VISA installer.\n'
	$process = Start-Process -FilePath "C:\temp\RS_VISA_Setup_Win_7_2_5.exe"  -PassThru
	dump2ScrnAndFile "installatieproces RS VISA met id: $($process.Id) loopt."

	# Wait untill process is finished
	Wait-Process -Id $process.Id
	dump2ScrnAndFile "installatie VISA voltooid."
	#getest op 8-12-2025. Dit werkt goed.
}

Function startJupyter {
	#start Jupyter notebook
	# is iets als: jupyter notebook notebook.ipynb, alleen moeten we dan de wynpython jupyter gebruiken.
	#ump2ScrnAndFile "Starten van Jupyter notebook server."
	#$process = Start-Process -FilePath "c:\Labcontrol\Jupyter Notebook.exe" -PassThru
	#dump2ScrnAndFile "Jupyter notebook server gestart met proces id: $($process.Id)."
	cmd.exe /c "c:\Labcontrol\Jupyter Notebook.exe"
}

dump2ScrnAndFile "installeren gestart"


if (Test-Path -Path $WinPyTargetPath) {
	dump2ScrnAndFile "Labcontrol map bestaat al op doelcomputer. Verwijderen voor nieuwe installatie."
}
else {
	dump2ScrnAndFile "Labcontrol map bestaat niet."
}
...
$skipDownload = prepareTempDir $dest 1
if ($skipDownload) {
