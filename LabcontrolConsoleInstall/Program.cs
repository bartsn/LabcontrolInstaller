...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        string fileName = "installfunctions.ps1";
        ...
        string fileName2 = "startJN.ps1";

        //string fileName = "LabcontrolInstaller.ps1";
        string combiPath2 = Path.Join(path2, fileName2);
        string psScriptPath2 = @combiPath2;

        var result2 = await ExecutePowerShellScriptJN(psScriptPath2);
    ...
    public static async Task<(string Output, int ExitCode)> ExecutePowerShellScriptAsync(string scriptPath)
    {
        try
        {
            ...
            string tempOutputFile = Path.Join(path, fileName);
            ...
            /* orignele wrappedcommand
             * string wrappedCommand = $@"
                try {{
                    $ErrorActionPreference = 'Continue'
                    $output = & '{scriptPath}' 3>&1 2>&1  | Out-String
                    $exitCode = $LASTEXITCODE
                    if ($null -eq $exitCode) {{ $exitCode = 0 }}
                    $output | Out-File -FilePath '{tempOutputFile}' -Encoding utf8
                    [System.Environment]::ExitCode = $exitCode
    ...
    public static async Task<(string Output, int ExitCode)> ExecutePowerShellScriptJN(string scriptPath)
    {
        try
        {
            // Create a temporary output file
            ...
            ProcessStartInfo psi = new ProcessStartInfo
            {
                FileName = "powershell.exe",
                Arguments = $"-ExecutionPolicy RemoteSigned -Command \"{wrappedCommand}\"",
                //Arguments = $"-ExecutionPolicy Bypass -NoProfile -NonInteractive -Command \"c:\\labcontrol\\Jupyter Notebook.exe\"",
