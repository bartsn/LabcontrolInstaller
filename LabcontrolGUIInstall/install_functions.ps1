...
$name = $args[0] 
$mycomm =[convert]::ToInt16($name)
switch ($mycomm) {
	0 { $gitAv = progExists "git"; return "0 $($gitAv)" }
	1 { $zipAv = progExists "7-Zip"; return "1 $($zipAv)" }
	2 {Get-PSDrive -PSProvider FileSystem; return "2"}
	4 { Get-CimInstance -Class Win32_LogicalDisk}
	5 {doInstall; return "5"}
	Default { dump2ScrnAndFile "onbekende optie" }
}
